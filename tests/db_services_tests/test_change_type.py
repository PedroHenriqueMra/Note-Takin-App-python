...
class TestChangeData(unittest.TestCase):
    ...
    def test_raise_exception(self):
        obj = ChangeInsert(self.table_type, self.table_id, "test")
...
class TestChangeInsertData(unittest.TestCase):
    def setUp(self):
        ...
        table_id = 1
        new_content = " new content"
        self.change:ChangeInsert = ChangeInsert(table_type, table_id, new_content)

    def test_get_query(self):
        change = self.change
        query_expected = f"UPDATE {change.table_type} SET content = content || ?"
        get_query = self.change.gen_change_script()

        print(f"\nInsert script:\n{get_query[0]}\nExpected:\n{query_expected[0]}")
        self.assertEqual(get_query[0], query_expected)
